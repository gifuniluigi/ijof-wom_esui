function [Y_forc,B,SIGMA,res] = BVARMINN_GLP(data,datax,p,delta,h,ndraws)

mm = zeros(1,size(data,2));
ss = ones(1,size(data,2));

% Run the Bayesian VAR
[res,Y,X,~,N] = bvarGLP_bk(data,datax,p,delta,1,2000,200,0);
...
X_FORE = [Y(end,:) X(end,2:N*(p-1)+1) X(end,end)];
BB = speye(N*p);
