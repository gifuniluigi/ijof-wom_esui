function [r,y,x,T,n] = bvarGLP_bk(y,x1,lags,delta,mcmc,Ndraws,Ndrawsdiscard,MCMCfcast)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% dimensions
[TT,n]=size(y);
k=n*lags+1+size(x1,2);         % # coefficients for each equation
...
x0=[-log((MAX.lambda-lambda0)/(lambda0-MIN.lambda));inpsi];

H0=10*eye(length(x0));          % initial guess for the inverse Hessian

%% maximization of the posterior of the hyperparameters
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
[~,xh,~,H,itct,~,~] = csminwel('logMLVAR_formin',x0,H0,[],1e-16,1000,y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,hyperpriors,priorcoef);
...
%[r.logML,r.betahat,r.epshat]=logMLVAR_forminEx(xh,y,x,lags,T,n,b,MIN,MAX,SS,mn);
...
if Fcast==1
    ...
    XX=x(:,end);
    for tau=1:max(hz)
        xT=[1;reshape(Y([T+tau-1:-1:T+tau-lags],:)',k-2,1);XX(T,:)]';
        ...
        if logMLnew>logMLold
            ...
            if rand(1)<exp(logMLnew-logMLold)
                ...
                if MCMCfcast==1 || MCMCstorecoeff==1
                    [~,betadrawnew,sigmadrawnew]=logMLVAR_formcmc(P(i,:)',y,x,lags,T,n,b,MIN,MAX,SS,Vc,pos,mn,sur,noc,y0,...
                        max([MCMCfcast,MCMCstorecoeff]),hyperpriors,priorcoef);
                end
            end
        end
