%%% This file estimates the results reported in Table E4, 4-VARX
...
%%% Real spot price West Texas Intermediate
    X = Uncertainty(1:irep,1);
    YF = [Y rwti(1:irep,1)];
    % Estimate VAR-OLS + rwti
    [beta_OLS,intercept,sigmaf,gamma] = ols_var(YF,r,nlag,X);
    y_fore(irep-t0+1,:,:,:,1) = fore_VAR(YF,intercept,beta_OLS,sigmaf,r,nlag,nfore,ndraws,gamma,X);
    clear YF X
...
%%% Crude Oil Refinery Acquisition Costs (Imported)
    ...
    [beta_OLS,intercept,sigmaf,gamma] = ols_var(YF,r,nlag,X);
    y_fore(irep-t0+1,:,:,:,2) = fore_VAR(YF,intercept,beta_OLS,sigmaf,r,nlag,nfore,ndraws,gamma,X);
    clear YF X
...
%%% Real spot price Brent
    ...
    [beta_OLS,intercept,sigmaf,gamma] = ols_var(YF,r,nlag,X);
    y_fore(irep-t0+1,:,:,:,3) = fore_VAR(YF,intercept,beta_OLS,sigmaf,r,nlag,nfore,ndraws,gamma,X);
    clear YF X
...
clear y_fore;
modname = '4VAR_X1'; % add _X1 (e.g., 4VAR_X1) if results are obtained using Scount;
                     % add _X2 (e.g., 4VAR_X2) if results are obtained using Dictionary: Financial Stability (Correa et al.[2017])
                     % and so on...
