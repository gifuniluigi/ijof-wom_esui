%% This code prints Panel B, CRPS 

% Round Table_DF to three decimals
roundedDF = round(Table_DF, 3);

% For each column, determine the minimum value
minVals = min(roundedDF, [], 1);
...
for i = 1:rows
    ...
    for j = 1:cols
        ...
        if abs(val - minVals(j)) < 1e-6   % Using tolerance for floating point equality
