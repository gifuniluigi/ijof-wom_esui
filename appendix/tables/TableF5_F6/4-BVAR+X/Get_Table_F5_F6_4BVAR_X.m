%%% This file estimates the results reported in Tables F5 & F6, 4-BVARX
clear; 
clc;
...
%%% Real spot price West Texas Intermediate
    ...
    y_fore(irep-t0+1,:,:,:,1) = BVARMINN_GLP(YF,X,nlag,0,nfore,ndraws);  
    clear YF
    
%%% Real spot price RAC
    YF = [Y rrac(1:irep,1)];
    % Estimate BVAR-OLS with rrac + Txt
    y_fore(irep-t0+1,:,:,:,2) = BVARMINN_GLP(YF,X,nlag,0,nfore,ndraws);  
    clear YF
     
%%% Real spot price Brent
    YF = [Y rbrent(1:irep,1)];    
    % Estimate BVAR-OLS with rbrent + Txt
    y_fore(irep-t0+1,:,:,:,3) = BVARMINN_GLP(YF,X,nlag,0,nfore,ndraws);  
    clear YF
...
clear y_fore;
modname = '4BVAR_X1'; % add _X1 (e.g., 4BVAR_X1) if results are obtained using Scount;
                      % add _X2 (e.g., 4BVAR_X2) if results are obtained using Dictionary: Financial Stability (Correa et al.[2017])
                      % and so on...
