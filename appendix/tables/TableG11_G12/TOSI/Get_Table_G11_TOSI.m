%%% This file estimates the results reported in Table G11, TOSI
...
sntind = 3;         % sentiment index methodology: 
                    % 1 = Stock and Watson (2005), 
                    % 2 = PCA
                    % 3 = 3-PRF Kelly and Pruitt (2015)
%-------------------------------LOAD DATA---------------------------------------
...
load Sentmtx                % Sample period: 1982.01 - 2021.06
...
for irep = t0:t-1
    ...
    if sntind == 1
        Sentiment = SW(bstvar(1:irep,:));
    elseif sntind == 2
        Sentiment = standardize(bstvar(1:irep,:));
        [~,Sentiment,~,~] = PCA(Sentiment,1);
    elseif sntind == 3
        XX = Sentmtx(1:irep,:)';
        Yrwti   = rwti(1:irep,:)';
        Yrrac   = rrac(1:irep,:)';
        Yrbrent = rbrent(1:irep,:)';
        Sent1   = PRF(XX, Yrwti);
        Sent2   = PRF(XX, Yrrac);
        Sent3   = PRF(XX, Yrbrent);
        Sentiment = [];
    end
    ...
    %%% Real spot price West Texas Intermediate
        ...
        if sntind == 3, YF = [Y rwti(1:irep,1) Sent1]; end
    ...
    %%% Real spot price RAC
        ...
        if sntind == 3, YF = [Y rrac(1:irep,1) Sent2]; end
    ...
    %%% Real spot price Brent
        ...
        if sntind == 3, YF = [Y rbrent(1:irep,1) Sent3]; end
        % Estimate BVAR-OLS with rrac + Txt
...
modname = 'PRF-SVBVAR'; % change according to sentiment index methodology used
save(sprintf('%s.mat',modname),'msfe','nfore','nmodel','TOSI');