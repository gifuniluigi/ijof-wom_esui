%%% This file estimates the results reported in Table G11, proxy SV-BVAR
...
proxy  = 'p1';      % proxy in the volatility variance
%-------------------------------LOAD DATA---------------------------------------
...
prior.e = [0 0 0 0.99];   % Prior means on the VAR part
...
%%% Real spot price West Texas Intermediate
    ...
    y_fore(irep-t0+1,:,:,:,1) = BVARMINN_CCM(YF,X,iprice,nlag,prior.e,prior.b,prior.c,prior.a,nfore,ndraws);
    clear YF
    
%%% Real spot price RAC
    YF = [Y rrac(1:irep,1)];
    % Estimate BVAR-OLS with rrac + Txt
    y_fore(irep-t0+1,:,:,:,2) = BVARMINN_CCM(YF,X,iprice,nlag,prior.e,prior.b,prior.c,prior.a,nfore,ndraws);
    clear YF
     
%%% Real spot price Brent
    YF = [Y rbrent(1:irep,1)];    
    % Estimate BVAR-OLS with rbrent + Txt
    y_fore(irep-t0+1,:,:,:,3) = BVARMINN_CCM(YF,X,iprice,nlag,prior.e,prior.b,prior.c,prior.a,nfore,ndraws);
    clear YF
...
clear y_fore;
modname = '4SVBVARp1';  % add p1 (e.g., 4SVBVARp1) if results are obtained using Scount;
                        % add p2 (e.g., 4SVBVARp2) if results are obtained using Dictionary: Financial Stability (Correa et al.[2017])
                        % and so on...
save(sprintf('%s%s.mat',modname,proxy),'msfe','nfore','nmodel','Sentiment');