function [Y_pred] = BVARMINN_CCM(data,datax,p,delta,lambda,theta,V_prior,h,ndraws)
%% ************************************************************************************
...
for i=1:N
    if i == 1
       sigma_const(i)=AR_s2(i,i)*theta(3); % this sets the prior variance on the intercept
        ...
        co=co+1;
        Pi_pv(co,co)=theta(1); % this sets the prior variance on the text variable
...
for m = 1:MCMCreps
    ...
    while stationary==0
        ...
        if (check_stationarity==0 || max(abs(eig([PAI(2:K-1,:)' ; comp]))) < 1); stationary = 1; end
    end
...
By = [B(2:end-1,:)'; eye(N*(p-1)) , zeros(N*(p-1),N)];
Txty = B(end,:);
Sy = zeros(N*p,N*p);
Sy(1:N,1:N) = SIGMA;
miu = zeros(N*p,1);
...
X_FORE = [Y(end,:) X(end,2:N*(p-1)+1) Xtest(end,:)];
BB = speye(N*p);
for ii = 1:h % not very efficient, By^(ii-1) can be defined once
    VAR_MEAN =  VAR_MEAN + BB*miu;
    Txt_FORE=[(Txty*X_FORE(:,end))';zeros(N*p-N,1)];
    FORECASTS = VAR_MEAN + (BB*By)*X_FORE(:,1:end-1)' + Txt_FORE;
    if ndraws > 1
