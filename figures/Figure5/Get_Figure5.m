%%% This file estimates the results reported in Figure 5
...
% Estimate all possible model combinations
for model = 1:comb
    ...
    prior.e = [Vpriors 0.99];               % Prior means on the VAR part
    [~,iprice] = find(prior.e==0.99);       % column of oil price in VAR
    ...
    for irep = t0:t-1
        ...
        if (-1)^model==-1, X=[]; else, X=Sentiment(1:irep,:); end
        if model<=2, Y=[]; else, Y=YY(1:irep,1:C(1,model-2)); end
    ...
    %%% Real spot price West Texas Intermediate
        ...
        y_fore(irep-t0+1,:,:,:,1) = BVARMINN_CCM(YF,nlag,prior.e,prior.b,prior.c,prior.a,nfore,ndraws,model);
        clear YF
        
    %%% Real spot price RAC
        YF = [Y rrac(1:irep,1) X];
        % Estimate BVAR-OLS with rrac + Txt
        y_fore(irep-t0+1,:,:,:,2) = BVARMINN_CCM(YF,nlag,prior.e,prior.b,prior.c,prior.a,nfore,ndraws,model);
        clear YF
        
    %%% Real spot price Brent
        YF = [Y rbrent(1:irep,1) X];
        % Estimate BVAR-OLS with rrac + Txt
        y_fore(irep-t0+1,:,:,:,3) = BVARMINN_CCM(YF,nlag,prior.e,prior.b,prior.c,prior.a,nfore,ndraws,model);
        clear YF
    ...
    clear y_fore;
    modname = 'SVBVARcmb';
    save(sprintf('%s%01d.mat',modname,model),'msfe','nfore','nmodel');
end
